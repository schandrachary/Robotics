...
int main( int argc, char** argv )
{
  ...
  while (ros::ok())
  {
    ...
    // Wait 5 seconds to simulate robot travel time to pickup location
    ros::Duration(5.0).sleep();

    // Delete pickup marker and display wait another 5 
    // seconds to simulate travel time to destination
    marker.action = visualization_msgs::Marker::DELETE;
    marker_pub.publish(marker);
    ROS_INFO("Pickup marker hidden");
    ros::Duration(5.0).sleep();

    // Draw marker at the drop off location simulating the robot
    // has dropped off the package
    ...
    marker.action = visualization_msgs::Marker::ADD;
    marker_pub.publish(marker);
    ROS_INFO("Dropoff marker published");
